...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        """
        #para hacer torch.bmm necesitamos vectores 3D
        output_vectors = output_vectors.unsqueeze(2) #num es el indice en el que insertar la nueva dimension
        input_vectors = input_vectors.unsqueeze(2)#(batch_size, embed_size) -> (batch_size, embed_size, 1)
        # Compute log-sigmoid loss for correct classifications
        # TODO
        pos_dot = torch.bmm(output_vectors.transpose(1, 2), input_vectors).squeeze(2)
        out_loss = torch.log(torch.sigmoid(pos_dot))

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        noise_dot = torch.bmm(noise_vectors, input_vectors).squeeze(2)
        noise_loss = torch.log(torch.sigmoid(-noise_dot))
